...
### どのカレンダーを読むか
このスクリプトは **スクリプトを所有するGoogleアカウント** として動く。
AXIS_SYNC_PRIVATE は個人Googleアカウントで作成したため、既定のままだと
個人アカウントのカレンダーを読む。対象が `tunagu.fukuoka@gmail.com` の場合は、

1. `tunagu.fukuoka@gmail.com` のカレンダーを、スクリプト所有アカウントへ共有する
   （Googleカレンダー → 設定 → 特定のユーザーとの共有 → 「予定の表示（すべての予定の詳細）」以上）
2. AXISのスケジュール画面 → **カレンダーを選ぶ** で一覧から選ぶ
   （`op=calendars` で見えるカレンダーを取得する。選択内容は端末内 `axis_calendar_id_v1` に保存）

共有していないカレンダーは一覧に出ず、指定してもエラーになる。
